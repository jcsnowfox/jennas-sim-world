...
- The BrowserWindow uses context isolation, disabled Node integration, sandboxing and web security. The preload exposes only `window.nightwater` methods for metadata, health reads, health refresh and typed health subscriptions.
- Renderer navigation is restricted to `nightwater://app/`, and `window.open` requests are denied.
- Renderer assets are resolved by a custom protocol resolver that rejects malformed URLs, unknown hosts, absolute paths, decoded traversal and encoded traversal.
- The Local API binds to `127.0.0.1` on an operating-system assigned ephemeral port and requires a per-launch bearer token generated by the Electron main process.
- Child processes are forked with explicit scripts and argument arrays, not shell execution. Shutdown uses graceful IPC first and a bounded forced-kill fallback.
